...
def main() -> int:
    ...
    try:
        ...
        jgenv.imem.update(init_bin)
        jgenv.rst.update(sim_fsdb)
        jgenv.restart_session()
        asserts = restore_asserts(runtime, jgenv)
        instr_bin = init_bin.copy()

        while not asserts.empty() and runtime.state["epoch"] < runtime.config.max_epochs:
            a = asserts.select_one()
            # debug
            # a = asserts.find("<embedded>::ibex_top.u_ibex_core.wb_stage_i.u_bsdcov_traversal_ibex_wb_stage_0bb5b39547.AST_BSDCOV_TRAVERSAL_ibex_wb_stage_0bb5b39547_040")
            retry = 0
            while (retry < runtime.config.retry_limit and
                   runtime.state["epoch"] < runtime.config.max_epochs):
                ...
                if candidate_bin is None:
                    ...
                    retry += 1
                    continue
                sim_fsdb, _candidate_coverage = sim_tool.sim(candidate_bin)
