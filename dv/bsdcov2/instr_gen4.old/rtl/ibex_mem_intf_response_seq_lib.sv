...
class ibex_mem_intf_response_seq extends uvm_sequence #(ibex_mem_intf_seq_item);
  ...
  function bit [7:0] read_byte(bit [ADDR_WIDTH-1:0] addr, output bit is_byte_uninit);
    bit [7:0] data = '0;
    logic [7:0] runtime_data;
    if (!is_dmem_seq &&
        instr_gen_runtime_imem_pkg::read_byte(addr, runtime_data)) begin
      `uvm_info(`gfn, $sformatf("Read runtime IMEM: Addr[0x%0h], Data[0x%0h]",
                                addr, runtime_data), UVM_HIGH)
      return runtime_data;
    end
