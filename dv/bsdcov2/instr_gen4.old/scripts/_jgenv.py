...
class JGRst:
    ...
    def tcl(self, work_dir: Path) -> str:
        ...
        reset_cmd = (
            f"reset -fsdb {_tcl_quote(self.fsdb.filepath)} "
            f"-hier_path {_tcl_quote(self.sim_hier)} "
        )
        if self.fsdb.reset_time_ps is not None:
            reset_cmd += f"-time {int(self.fsdb.reset_time_ps)} -time_scale ps "
        reset_cmd += "-non_resettable_regs 0"
...
""", encoding="utf-8")
    ...
    def restart_session(self) -> None:
        ...
        self.close()
        epoch = int(self.runtime.state.get("epoch", 0))
        if epoch <= 0:
            raise RuntimeError("cannot restart Jasper before an epoch is selected")
