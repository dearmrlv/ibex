...
RETIRED = re.compile(
    r"BSDCOV_EVAL_RETIRED (?:time_ps=(\d+) )?"
    r"pc=0x([0-9a-fA-F]{8}) insn=0x([0-9a-fA-F]{8})"
)
BLOCKED = re.compile(r"BSDCOV_EVAL_RETIRED .* blocked_req_seen=([01])")
ERROR_HANDLED = re.compile(
    r"BSDCOV_EVAL_ERROR_HANDLED (?:time_ps=(\d+) )?"
    r".*?error_count=(\d+).*?wait_cycles=(\d+)"
)
STOP_TIME = re.compile(r"^\s*(\d+)\s+PS\s+\+\s+\d+\s+\(stop bsdcov_epoch_done:", re.MULTILINE)
TRACE = re.compile(
...
def main() -> int:
    ...
    markers = [(int(pc, 16), int(insn, 16)) for _time, pc, insn in retired_matches]
    ...
    error_handled_markers = [(int(count), int(wait))
                             for _time, count, wait in error_matches]
    error_handled = bool(error_handled_markers)
    stop_times = [int(time) for time in STOP_TIME.findall(text)]
    marker_times = stop_times
    if not marker_times:
        marker_times = [int(time) for time, _pc, _insn in retired_matches if time]
        marker_times.extend(int(time) for time, _count, _wait in error_matches if time)
    snapshot_time_ps = marker_times[-1] if marker_times else None
    ...
    if not error_handled and trace and trace[-1] != (stop_pc, stop_instruction):
    ...
    bounded_marker = (
        ...
        "BSDCOV_RUNTIME_IMEM loaded" in text
    )
