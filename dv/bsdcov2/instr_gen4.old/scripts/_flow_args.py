...
class FlowConfig:
    ...
    def from_state(cls, state: dict[str, Any]) -> "FlowConfig":
        ...
        config.setdefault("sim_mode", "checkpoint")
...
class FlowArgs:
    ...
    def arguments(self, argv: Sequence[str] | None = None) -> argparse.Namespace:
        ...
        parser.add_argument("--sim-mode", choices=("checkpoint", "oneshot"),
                            default="checkpoint",
                            help="candidate simulation mode; checkpoint reuses an xrun session with save/restart")
