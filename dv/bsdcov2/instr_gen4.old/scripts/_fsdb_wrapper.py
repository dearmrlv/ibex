...
class FSDBWrapper:
    def __init__(self, filepath: Path, source: FSDBSource | str,
        ...
        self.reset_time_ps = reset_time_ps
    ...
    def from_simulation(cls, filepath: Path, verdi: Path = DEFAULT_VERDI,
                        reset_time_ps: int | None = None) -> "FSDBWrapper":
        return cls(filepath, FSDBSource.SIM, verdi, reset_time_ps=reset_time_ps)
    ...
    def from_dict(cls, value: dict[str, Any]) -> "FSDBWrapper":
        return cls(Path(value["filepath"]), FSDBSource(value["source"]),
                   ...
                   reset_time_ps=value.get("reset_time_ps"))
    ...
    def to_dict(self) -> dict[str, str | int]:
        ...
        if self.reset_time_ps is not None:
            result["reset_time_ps"] = self.reset_time_ps
        return result
