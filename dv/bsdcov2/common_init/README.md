...
This self-contained image executes the architectural initialization sequence
from `0x80000080` through `0x8000011c`. A simulation-only RVFI
...
handler. The linker defines `trap_handler` as `0x80000138` so the final `mtvec`
value is deterministic. Building and running do not depend on `bsdcov1`.

```bash
cd designs/ibex/dv/bsdcov2/common_init
make run
```
