...
class SimTool:
    ...
    def _check_result(self, run_dir: Path, driver_exit_code: int,
        ...
        check = self.runtime.paths.eval / "scripts/_check_result.py"
    ...
    def _run_shell(command: str, cwd: Path, env: dict[str, str], log: Path,
        ...
        process = subprocess.Popen(
            args, cwd=cwd, env=merged, text=True, stdout=subprocess.PIPE,
        ...
        try:
            ...
            stdout = SimTool._to_text(exc.output)
            SimTool._terminate_process_group(process)
            rest, _ = process.communicate()
            stdout += SimTool._to_text(rest)
            stdout += (
                f"\nBSDCOV: command timed out after {timeout}s; "
                "terminated process group\n")
            returncode = 124
        except KeyboardInterrupt:
            SimTool._terminate_process_group(process)
            rest, _ = process.communicate()
            log.write_text(SimTool._to_text(rest), encoding="utf-8", errors="ignore")
            raise
        process_result = subprocess.CompletedProcess(args, returncode, stdout=stdout)
        log.write_text(stdout, encoding="utf-8", errors="ignore")
        if check and process_result.returncode:
            raise RuntimeError(f"command failed ({process_result.returncode}); see {log}")
