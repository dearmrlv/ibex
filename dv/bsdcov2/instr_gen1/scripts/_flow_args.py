...
class FlowConfig:
    ...
    def from_args(cls, args: argparse.Namespace) -> "FlowConfig":
        ...
        return cls(
            ...
            exclude=normalize_excludes(args.exclude),
            retry_limit=args.retry_limit,
            prove_timeout=args.prove_timeout,
            prop_batch_size=args.prop_batch_size,
    ...
    def from_state(cls, state: dict[str, Any]) -> "FlowConfig":
        ...
        config.setdefault("prop_batch_size", 8)
        if config["target_sample_num"] == 0:
            config["target_sample_num"] = -1
        config["exclude"] = normalize_excludes(config["exclude"])
        return cls(**config)
...
def normalize_excludes(values: Sequence[str] | None) -> list[str]:
    result: list[str] = []
    seen: set[str] = set()
...
class FlowArgs:
    ...
    def arguments(self, argv: Sequence[str] | None = None) -> argparse.Namespace:
        ...
        parser.add_argument("--exclude", action="append", default=[],
                            help="RTL hierarchy to exclude from target extraction; may be repeated")
        parser.add_argument("--retry-limit", type=int, default=5)
        parser.add_argument("--prove-timeout", type=int, default=300)
        parser.add_argument("--prop-batch-size", type=int, default=8,
                            help="number of target properties to prove in one Jasper batch")
    ...
    def validate(self, args: argparse.Namespace) -> None:
        positive = {
            ...
            "prop-batch-size": args.prop_batch_size,
