...
def save_sigset(runtime: FlowRuntime, sigset: SigSet) -> None:
    runtime.state["sigset"] = sigset.to_state()
    runtime.state["sigset_summary"] = {
        "signal_count": len(sigset.signals),
        "remaining_disappeared_values": remaining_disappears(sigset),
    }
    save(runtime)


def _accepted_fsdb(runtime: FlowRuntime, relative: str) -> FSDBWrapper:
    run = runtime.paths.run_dir / relative
    result_path = run / "result.json"
    snapshot_time_ps = None
    if result_path.is_file():
        result = json.loads(result_path.read_text(encoding="utf-8"))
        snapshot_time_ps = result.get("snapshot_time_ps")
    return FSDBWrapper.from_simulation(run / "fsdb/eval.fsdb",
                                       snapshot_time_ps=snapshot_time_ps)


def restore_sigset(runtime: FlowRuntime, sim_fsdb: FSDBWrapper) -> SigSet:
    state = runtime.state.get("sigset")
    if state:
        sigset = SigSet.from_state(state, fsdb=sim_fsdb.filepath)
        save_sigset(runtime, sigset)
        return sigset

    accepted = list(runtime.state.get("accepted_runs", []))
    if not accepted:
        sigset = make_sigset(runtime, sim_fsdb)
        save_sigset(runtime, sigset)
        return sigset

    base_fsdb = _accepted_fsdb(runtime, accepted[0])
    sigset = make_sigset(runtime, base_fsdb)
    for relative in accepted[1:]:
        sigset.update_from_fsdb_incremental(_accepted_fsdb(runtime, relative).filepath)
    save_sigset(runtime, sigset)
    return sigset
...
def initial_sim(runtime: FlowRuntime, sim_tool: SimTool, init_bin: SimBin,
    ...
    runtime.state["sim_checkpoint"] = sim_tool.last_result["checkpoint"]
...
def accept_candidate(runtime: FlowRuntime, sim_tool: SimTool, jgenv: JGEnv,
    ...
    checkpoint = sim_tool.commit_checkpoint()
    ...
    runtime.state["sim_checkpoint"] = checkpoint
...
def mark_failed(runtime: FlowRuntime, exc: BaseException) -> None:
    runtime.state["status"] = "failed"
    runtime.state["error"] = str(exc)
    save(runtime)
    atomic_json(runtime.paths.final / "run_summary.json", {
        "status": "failed",
        "accepted": runtime.state.get("accepted", 0),
        "iterations": runtime.state.get("iteration", 0),
        "epoch": runtime.state.get("epoch", 0),
        "error": str(exc),
        "current_image": runtime.state.get("current_image"),
        "coverage": runtime.state.get("coverage"),
    })
...
def main() -> int:
    ...
    try:
        ...
        sigset = restore_sigset(runtime, sim_fsdb)
            ...
            while (not asserts.empty() and
                ...
                while retry < runtime.config.retry_limit:
                    ...
                    if new_coverage.higher_than(coverage):
                        ...
                        sigset.update_from_fsdb_incremental(sim_fsdb.filepath)
                        save_sigset(runtime, sigset)
