...
class JGRst:
    ...
    def tcl(self, work_dir: Path) -> str:
        ...
        if self.fsdb.snapshot_time_ps is None:
            raise RuntimeError("simulation FSDB is missing snapshot_time_ps for Jasper reset")
        self.jg_rst_file = work_dir / "init_state.rst"
        return f"""set init_state [file join $work_dir init_state.rst]
reset -clear
reset -fsdb {_tcl_quote(self.fsdb.filepath)} -hier_path {_tcl_quote(self.sim_hier)} -time {self.fsdb.snapshot_time_ps} -time_scale ps -non_resettable_regs 0
...
""", encoding="utf-8")
    ...
    def restart_session(self) -> None:
        ...
        self.close()
        epoch = int(self.runtime.state.get("epoch", 0)) + 1
