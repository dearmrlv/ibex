...
class ibex_mem_intf_response_driver extends uvm_driver #(ibex_mem_intf_seq_item);
  ...
  virtual protected task send_grant();
    int gnt_delay;
    bit limit_common_init_imem;
    limit_common_init_imem = !cfg.enable_bad_intg_on_uninit_access;
    $display("BSDCOV_EVAL_MEM_DRIVER name=%s bounded=%0d range=0x80000080..0x%08x",
             get_full_name(), limit_common_init_imem, cfg.vif.bsdcov_imem_last_addr);
    forever begin
      while(cfg.vif.response_driver_cb.request !== 1'b1 ||
            (limit_common_init_imem &&
             !(cfg.vif.response_driver_cb.addr inside
               {[32'h8000_0080:cfg.vif.bsdcov_imem_last_addr]}))) begin
      ...
      if(~cfg.vif.response_driver_cb.reset &&
         cfg.vif.response_driver_cb.request === 1'b1 &&
         (!limit_common_init_imem ||
          (cfg.vif.response_driver_cb.addr inside
           {[32'h8000_0080:cfg.vif.bsdcov_imem_last_addr]}))) begin
