...
module core_ibex_tb_top;
  ...
  ibex_mem_intf data_mem_vif(
    .clk                    (clk),
    .limit_common_init_imem (1'b0)
  );
  ibex_mem_intf instr_mem_vif(
    .clk                    (clk),
    .limit_common_init_imem (1'b1)
  );
...
`endif
  ...
  bsdcov_instr_gen_runtime_ctrl u_bsdcov_runtime_ctrl();

  task automatic bsdcov_patch_bin_suffix(input string bin, input logic [31:0] patch_start);
    uvm_component comp;
    core_ibex_base_test test_h;
    bit [7:0] r8;
    bit [31:0] addr;
    int bin_fd;
    int patched_bytes;

    comp = uvm_top.find("uvm_test_top");
    if (!$cast(test_h, comp)) begin
      $fatal(1, "BSDCOV cannot cast uvm_test_top to core_ibex_base_test");
    end
    ...
    bin_fd = $fopen(bin, "rb");
    if (!bin_fd) begin
      $fatal(1, "BSDCOV cannot open binary %0s", bin);
    end
    while ($fread(r8, bin_fd)) begin
      if (addr >= patch_start) begin
        test_h.mem.write(addr, r8);
        patched_bytes++;
      end
      addr++;
    end
    $fclose(bin_fd);
    $display("BSDCOV_PATCH_BIN_SUFFIX bin=%0s patch_start=0x%08x patched_bytes=%0d",
             bin, patch_start, patched_bytes);
  endtask

  task automatic bsdcov_apply_runtime_control(
    input string bin,
    input logic [31:0] patch_start,
    input logic [31:0] last_addr,
    input logic [31:0] stop_pc,
    input logic [31:0] stop_instruction
  );
    bsdcov_patch_bin_suffix(bin, patch_start);
    instr_mem_vif.bsdcov_set_last_addr(last_addr);
    u_bsdcov_instr_gen_eval_stop.bsdcov_config_stop(stop_pc, stop_instruction);
  endtask
endmodule
