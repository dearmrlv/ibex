...
class FlowConfig:
    ...
    disable_per_epoch_coverage_merge: bool
    disable_per_sim_imc_report: bool
    jg: str
    created_at: str
    ...
    def from_args(cls, args: argparse.Namespace) -> "FlowConfig":
        ...
        return cls(
            ...
            if args.prove_timeout is not None else DEFAULT_PROVE_TIMEOUT,
            max_epochs=args.max_epochs,
            max_generated_words=args.max_generated_words,
            disable_per_epoch_coverage_merge=args.disable_per_epoch_coverage_merge,
            disable_per_sim_imc_report=args.disable_per_sim_imc_report,
    ...
    def from_state(cls, state: dict[str, Any]) -> "FlowConfig":
        ...
        config.setdefault("disable_per_epoch_coverage_merge", False)
        config.setdefault("disable_per_sim_imc_report", False)
...
class FlowArgs:
    ...
    def arguments(self, argv: Sequence[str] | None = None) -> argparse.Namespace:
        ...
        parser.add_argument("--prove-timeout", type=int, default=None)
        parser.add_argument("--max-epochs", type=int, default=100)
        parser.add_argument("--max-generated-words", type=int, default=4096)
        parser.add_argument("--disable-per-epoch-coverage-merge", action="store_true",
                            help="use candidate simulation coverage directly instead of "
                                 "IMC-merging all accepted UCDs after every candidate")
        parser.add_argument("--disable-per-sim-imc-report", action="store_true",
                            help="skip per-simulation IMC report generation; candidate "
                                 "acceptance is based on RTLReach target reduction")
    ...
    def prepare(self, args: argparse.Namespace) -> FlowRuntime:
        ...
        if args.resume:
            ...
            if args.prove_timeout is not None:
                config = replace(config, prove_timeout=args.prove_timeout)
                config_changed = True
            if args.retry_limit != config.retry_limit:
                config = replace(config, retry_limit=args.retry_limit)
                config_changed = True
            if args.max_epochs != config.max_epochs:
                config = replace(config, max_epochs=args.max_epochs)
                config_changed = True
            if config_changed:
                state["config"] = config.to_json()
