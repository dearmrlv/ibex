...
class SimTool:
    ...
    def sim(self, instr_bin: SimBin) -> tuple[FSDBWrapper, CovData | None]:
        ...
        return self.sim_at(instr_bin, run_dir, update_last=True)

    def sim_at(self, instr_bin: SimBin, run_dir: Path,
               update_last: bool = False) -> tuple[FSDBWrapper, CovData | None]:
    ...
    def _check_result(self, run_dir: Path, driver_exit_code: int,
        ...
        elif "alert_assertion_failure" in failures:
            result["status"] = "fail_with_alert"
            atomic_json(result_path, result)
