...
module bsdcov_instr_gen_eval_stop (
  ...
  localparam int unsigned POST_RETIRE_WAIT_CYCLES = 20;
  ...
  always @(posedge clk_i) begin
    ...
    if (rvfi_valid_i && rvfi_pc_i == stop_pc) begin
      ...
      begin
        bit next_fetch_seen;
        int unsigned wait_cycle;
        next_fetch_seen = 1'b0;
        for (wait_cycle = 0; wait_cycle < POST_RETIRE_WAIT_CYCLES; wait_cycle++) begin
          @(posedge clk_i);
          if (instr_req_i && instr_addr_i == (retired_pc + 32'd4)) begin
            next_fetch_seen = 1'b1;
          end
        end
        #1ps;
        $display("BSDCOV_EVAL_NEXT_FETCH expected=0x%08x seen=%0d wait_cycles=%0d last_req=0x%08x",
                 retired_pc + 32'd4, next_fetch_seen, POST_RETIRE_WAIT_CYCLES, instr_addr_i);
      end
      $finish;
    end
  end
