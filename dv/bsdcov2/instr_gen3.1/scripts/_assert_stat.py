...
class AssertCex:
    ...
    def _extract(self) -> list[InstrWrapper]:
        ...
        candidate = [InstrWrapper(address, word) for address, word in sorted(new.items())]
        for index, instr in enumerate(candidate):
            expected = start + index * 4
            if instr.addr != expected:
                raise RuntimeError(f"CEX instruction addresses are sparse: expected "
                                   f"0x{expected:08x}, got 0x{instr.addr:08x}")
        return candidate
...
class AssertCollection:
    ...
    def select_one(self) -> AssertStat:
        ...
        return min(selectable, key=lambda assertion: assertion.name)
