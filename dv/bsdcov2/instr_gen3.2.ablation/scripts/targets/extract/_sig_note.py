...
class SigSet:
    ...
    def count_sva_properties(self, mode: str = "merged") -> int:
        pairs = [(signal, missing) for signal in self.signals
                 for missing in signal.disappear_vals]
        if mode == "merged":
            return len({signal.rtl_instance for signal, _ in pairs})
        if mode == "individual":
            return len(pairs)
        raise ValueError(f"unknown SVA property mode: {mode}")

    def dump_sva(self, file: Path, mode: str = "merged") -> Path:
        if mode not in {"merged", "individual"}:
            raise ValueError(f"unknown SVA property mode: {mode}")
        ...
        for group_index, (rtl_instance, group_pairs) in enumerate(sorted(groups.items())):
            ...
            lines.append("")
            disable = (f"(!{self.reset})" if self.reset_active_low else f"({self.reset})")
            if mode == "merged":
                lines += [f"  {property_name}: assert property (",
                          f"    @(posedge {self.clock}) disable iff {disable}",
                          "      !(" + " || ".join(f"fire_{index}" for index in range(len(group_pairs))) + ")",
                          "  );"]
            else:
                for index in range(len(group_pairs)):
                    lines += [f"  AST_BSDCOV_individual_{digest}_{index:04d}: assert property (",
                              f"    @(posedge {self.clock}) disable iff {disable}",
                              f"      !(fire_{index})",
                              "  );"]
                    if index + 1 < len(group_pairs):
                        lines.append("")
            lines += ["", "endmodule", ""]
