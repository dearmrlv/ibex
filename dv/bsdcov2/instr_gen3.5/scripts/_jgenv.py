...
class JGIMem:
    ...
    def gen_sv(self, output: Path) -> Path:
        ...
        end = start + 4 * MAX_SYMBOLIC_WORDS
        ...
        output.write_text(template.render(
            ...
            symbolic_end=_hx(end),
            max_symbolic_words=MAX_SYMBOLIC_WORDS,
            max_loop_fetches=MAX_LOOP_FETCHES,
...
class JGEnv:
    ...
    def _prepare_env(self, work_dir: Path) -> Path:
        ...
        symbolic_end = symbolic_start + 4 * MAX_SYMBOLIC_WORDS
        environment = Environment(
            loader=FileSystemLoader(TEMPLATE_DIR),
            undefined=StrictUndefined,
            autoescape=False,
