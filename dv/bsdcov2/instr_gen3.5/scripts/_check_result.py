...
NEXT_FETCH = re.compile(
    r"BSDCOV_EVAL_NEXT_FETCH expected=0x([0-9a-fA-F]{8}) seen=([01])"
    r".*?last_req=0x([0-9a-fA-F]{8})"
)
TRACE = re.compile(
...
def main() -> int:
    ...
    next_fetch_markers = [
        (int(expected, 16), seen == "1", int(last_req, 16))
        for expected, seen, last_req in NEXT_FETCH.findall(text)
    ]
    ...
    alert_assertion_failures = len(re.findall(r"NoAlertsTriggered", text))
    if alert_assertion_failures:
        failures.append("alert_assertion_failure")
    result = {
        ...
        "next_fetch_marker": bool(next_fetch_markers and next_fetch_markers[-1][1]),
        "next_fetch_expected": (
            f"0x{next_fetch_markers[-1][0]:08x}" if next_fetch_markers else None
        ),
        "next_fetch_last_req": (
            f"0x{next_fetch_markers[-1][2]:08x}" if next_fetch_markers else None
        ),
        ...
        "alert_assertion_failures": alert_assertion_failures,
