...
def _normalize_excludes(values: Sequence[str] | None) -> list[str]:
    result: list[str] = []
    seen: set[str] = set()
...
def _is_excluded(instance: str, excludes: Sequence[str]) -> bool:
    return any(instance == item or instance.startswith(item + ".")
...
class SigSet:
    def __init__(
        ...
        *,
        ...
        self.exclude = _normalize_excludes(exclude)
    ...
    def extract(self) -> None:
        ...
        for item in inventory["signals"]:
            ...
            if _is_excluded(rtl_instance, self.exclude):
                continue
        ...
        observed_by_name = self._observed_values_for_notes(pending)
        notes: list[SigNote] = []
    ...
    def update_from_fsdb(self, fsdb: Path | None = None) -> None:
        if fsdb is not None:
            ...
            if not self.fsdb.is_file():
                raise FileNotFoundError(f"FSDB does not exist: {self.fsdb}")
        observed_by_name = self._observed_values_for_notes(
            [(signal, signal.hier_name) for signal in self.signals]
        )
    ...
    def _observed_values_for_notes(
        self,
        notes: Sequence[tuple[SigNote, str]],
    ) -> dict[str, set[str]]:
        result = self._observed_values_batch(
            [(fsdb_name, note.width) for note, fsdb_name in notes]
        )
        supplemented = 0
        unresolved = 0
        for note, fsdb_name in notes:
            observed = result.setdefault(fsdb_name, set())
            legal_values = {
                bits for value in note.legal_values
                if (bits := _normalize_bits(value.val, note.width)) is not None
            }
            if legal_values.issubset(observed):
                continue
            try:
                debug_values = self._observed_values(fsdb_name, note.width)
            except RuntimeError as exc:
                unresolved += 1
                self.diagnostics.append(
                    f"FSDB value fallback failed for {fsdb_name}: {exc}")
                continue
            before = set(observed)
            observed.update(debug_values)
            if observed != before:
                supplemented += 1
            if not debug_values and not legal_values.issubset(observed):
                unresolved += 1
                self.diagnostics.append(
                    f"FSDB value fallback found no values for {fsdb_name}")
        if supplemented:
            self.diagnostics.append(
                f"FSDB value extraction supplemented {supplemented} signal(s) with fsdbdebug -vc")
        if unresolved:
            self.diagnostics.append(
                f"FSDB value extraction left {unresolved} signal(s) without complete observed values")
        return result
