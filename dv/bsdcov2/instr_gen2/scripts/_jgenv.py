...
class JGEnv:
    ...
    def call_proof(self, assertion: AssertStat) -> None:
        ...
        retry_index = len(assertion.cexs)
        self._require_session()
        work_dir.mkdir(parents=True, exist_ok=True)
        ...
        try:
            self._run_job(tcl, work_dir, self.runtime.config.prove_timeout + 5)
        except Exception:
        ...
        if not traces:
            assertion.set_status(ProofStatus.ERROR)
            raise RuntimeError(f"Jasper returned {raw_status} without an FSDB trace")
    ...
    def call_proof_batch(self, assertions: list[AssertStat],
                         batch_index: int) -> tuple[AssertStat | None, dict[str, str]]:
        if not assertions:
        ...
        if iteration <= 0:
            raise ValueError("proof requires a positive state iteration")
        self._require_session()
        work_dir = (self.runtime.paths.iterations / f"{iteration:04d}" /
                    "formal" / f"batch_{batch_index:04d}")
        work_dir.mkdir(parents=True, exist_ok=True)
        tcl = work_dir / "prove_batch.tcl"
        self._emit_batch_job_tcl(tcl, work_dir, assertions)
        traces_before = {
            path.resolve(): path.stat().st_mtime_ns
            for path in (work_dir / "traces").glob("**/*.fsdb")
        }
        ...
        status_by_name = self._read_batch_status(work_dir / "status.csv")
        cex_names = sorted(
            name for name, status in status_by_name.items()
            if status in {"cex", "ar_cex"}
        )
        winner: AssertStat | None = None
        for assertion in assertions:
            raw_status = status_by_name.get(assertion.name, "error")
            if raw_status in {"proven", "pass"}:
                if assertion.cexs:
                    assertion.proof_status = ProofStatus.PROVEN
                else:
                    assertion.set_status(ProofStatus.PROVEN)
            elif raw_status in {"timeout", "inconclusive", "undetermined"}:
                assertion.set_status(ProofStatus.TIMEOUT)
            elif raw_status in {"cex", "ar_cex"}:
                assertion.set_status(ProofStatus.PENDING)
            else:
                assertion.set_status(ProofStatus.ERROR)

        if cex_names:
            winner_name = cex_names[0]
            winner = next(
                assertion for assertion in assertions
                if assertion.name == winner_name
            )
            traces = sorted(
                (path for path in (work_dir / "traces").glob("**/*.fsdb")
                 if path.resolve() not in traces_before or
                ...
                key=lambda path: path.stat().st_mtime_ns)
            if not traces:
                winner.set_status(ProofStatus.ERROR)
                raise RuntimeError(
                    f"Jasper returned CEX for {winner.name} without an FSDB trace")
            output_csv = (self.runtime.paths.iterations / f"{iteration:04d}" /
                          "candidate" / f"batch_{batch_index:04d}.words.csv")
            winner.add_cex(AssertCex(
                FSDBWrapper.from_formal(traces[-1]), self.imem.bin, output_csv))
        return winner, status_by_name
...
close $fd"""
  ...
  -time_limit {self.runtime.config.prove_timeout}s \\
...
""", encoding="utf-8")
    ...
    def _emit_batch_job_tcl(self, output: Path, work_dir: Path,
                            assertions: list[AssertStat]) -> None:
        target_props = " ".join(_tcl_quote(assertion.name) for assertion in assertions)
        output.write_text(f"""set work_dir {_tcl_quote(work_dir)}
file mkdir $work_dir
file mkdir [file join $work_dir traces]
set target_props [list {target_props}]
set_stop_on_cex_limit 1
prove -property $target_props -asserts -force -engine_mode auto \\
  -time_limit {self.runtime.config.prove_timeout}s \\
  -per_property_time_limit {self.runtime.config.prove_timeout}s \\
  -dump_trace \\
  -dump_trace_type fsdb \\
  -dump_trace_dir [file join $work_dir traces]
set_stop_on_cex_limit 0
set status_fd [open [file join $work_dir status.csv] w]
puts $status_fd "property,status"
set cex_fd [open [file join $work_dir cex_properties.txt] w]
foreach p $target_props {{
  set status [get_status $p]
  puts $status_fd "$p,$status"
  if {{$status eq "cex" || $status eq "ar_cex"}} {{
    puts $cex_fd $p
  }}
}}
close $status_fd
...
""", encoding="utf-8")
    ...
    @staticmethod
    def _read_batch_status(path: Path) -> dict[str, str]:
        if not path.is_file():
            raise RuntimeError(f"Jasper did not produce batch status file: {path}")
        with path.open(newline="", encoding="utf-8") as stream:
            return {
                row["property"]: row["status"].strip().lower()
    ...
    def restart_session(self, epoch: int) -> None:
...
""", encoding="utf-8")
    ...
    @staticmethod
    def _terminate_process_group(process: subprocess.Popen[bytes],
        ...
        try:
            ...
            LOGGER.error("JasperGold process group did not exit after SIGKILL: pgid=%s",
                         process.pid)
