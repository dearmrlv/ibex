...
def main() -> int:
    ...
    if not markers or any(marker != (stop_pc, stop_instruction) for marker in markers):
        failures.append("missing_or_wrong_retirement_marker")
    if not trace or trace[-1] != (stop_pc, stop_instruction): failures.append("wrong_last_retired_instruction")
