...
RUN_TAG="${RUN_TAG:-mc.$(date -u +%H%M%S).s${SEED}.n${BASELINE_INSTR}}"
RUN_DIR="$BSDCOV_DIR/runs/$RUN_TAG"
PROJ_DIR="$RUN_DIR/big_proj"
REPORT_DIR="$RUN_DIR/reports"
STRICT_EXTRACT="${STRICT_EXTRACT:-1}"
...
extract_one() {
  local label="$1"
  local module="$2"
  local dut_inst_hier="$3"
  local output_signals="$4"

  echo
  echo "================================================================"
  echo "BSD-COV EXTRACT: $label"
  echo "  module        : $module"
  echo "  dut_inst_hier : $dut_inst_hier"
  echo "  output_signals: $output_signals"
  echo "================================================================"

  if python3 "$ROOT/main.py" bsdcov extract \
      --module "$module" \
      --dut-inst-hier "$dut_inst_hier" \
    ...
    echo "[FAIL] $label" >&2
    extract_failures+=("$label")
    if [[ "$STRICT_EXTRACT" != "0" ]]; then
      exit 1
    fi
  fi
}
...
extract_one \
  "if_stage.fetch_ctrl" \
  "ibex_if_stage" \
  "ibex_top.u_ibex_core.if_stage_i" \
  "instr_req_o,instr_intg_err_o,if_busy_o"

extract_one \
  "id_stage.stall_ctrl" \
  "ibex_id_stage" \
  "ibex_top.u_ibex_core.id_stage_i" \
  "stall_mem,stall_ld_hz,stall_multdiv,stall_branch,stall_jump,stall_id,stall_wb,flush_id,multicycle_done,instr_done,instr_executing,instr_executing_spec,controller_run"

extract_one \
  "id_stage.branch_jump_ctrl" \
  "ibex_id_stage" \
  "ibex_top.u_ibex_core.id_stage_i" \
  "branch_in_dec,branch_set,branch_set_raw,branch_set_raw_d,branch_jump_set_done_d,branch_taken,jump_in_dec,jump_set_dec,jump_set,jump_set_raw"
# branch_not_set is intentionally not an endpoint here: it is a fanin boundary
# for this cone, and making it both input and output produces ambiguous IO CSV
# columns in the current conegen/cook data model.

extract_one \
  "id_stage.multdiv_ctrl_only" \
  "ibex_id_stage" \
  "ibex_top.u_ibex_core.id_stage_i" \
  "mult_en_ex_o,div_en_ex_o,mult_sel_ex_o,div_sel_ex_o,multdiv_operator_ex_o,multdiv_signed_mode_ex_o,multdiv_ready_id_o,mult_en_id,div_en_id,mult_en_dec,div_en_dec,multdiv_en_dec"

extract_one \
  "id_stage.lsu_ctrl_only" \
  "ibex_id_stage" \
  "ibex_top.u_ibex_core.id_stage_i" \
  "lsu_req_o,lsu_we_o,lsu_type_o,lsu_sign_ext_o,lsu_req_dec,lsu_we,lsu_type,lsu_sign_ext,lsu_req,data_req_allowed,expecting_load_resp_o,expecting_store_resp_o,mem_resp_intg_err"

extract_one \
  "controller.fsm_ctrl" \
  "ibex_controller" \
  "ibex_top.u_ibex_core.id_stage_i.controller_i" \
  "ctrl_fsm_ns,stall,halt_if,retain_id,flush_id,exc_req_lsu,special_req,special_req_pc_change,special_req_flush_only,id_wb_pending,handle_irq,irq_enabled,enter_debug_mode,ebreak_into_debug"

extract_one \
  "ex_block.branch_valid_ctrl" \
  "ibex_ex_block" \
  "ibex_top.u_ibex_core.ex_block_i" \
  "branch_decision_o,ex_valid_o,alu_cmp_result,alu_is_equal_result,multdiv_valid,multdiv_sel"

extract_one \
  "wb_stage.ready_hazard_ctrl" \
  "ibex_wb_stage" \
  "ibex_top.u_ibex_core.wb_stage_i" \
  "ready_wb_o,rf_write_wb_o,outstanding_load_wb_o,outstanding_store_wb_o"

extract_one \
  "wb_stage.retire_ctrl" \
  "ibex_wb_stage" \
  "ibex_top.u_ibex_core.wb_stage_i" \
  "perf_instr_ret_wb_o,perf_instr_ret_compressed_wb_o,perf_instr_ret_wb_spec_o,perf_instr_ret_compressed_wb_spec_o,rf_we_wb_o,dummy_instr_wb_o,instr_done_wb_o"

extract_one \
  "decoder.trap_special" \
  "ibex_decoder" \
  "ibex_top.u_ibex_core.id_stage_i.decoder_i" \
  "illegal_insn_o,ebrk_insn_o,mret_insn_o,dret_insn_o,ecall_insn_o,wfi_insn_o"

extract_one \
  "decoder.jump_icache" \
  "ibex_decoder" \
  "ibex_top.u_ibex_core.id_stage_i.decoder_i" \
  "jump_set_o,icache_inval_o,jump_in_dec_o,branch_in_dec_o"

extract_one \
  "decoder.rf_ctrl_only" \
  "ibex_decoder" \
  "ibex_top.u_ibex_core.id_stage_i.decoder_i" \
  "rf_wdata_sel_o,rf_we_o,rf_ren_a_o,rf_ren_b_o"

if [[ "${#extract_failures[@]}" -ne 0 ]]; then
  printf 'ERROR: %d cone extraction(s) failed:\n' "${#extract_failures[@]}" >&2
  printf '  %s\n' "${extract_failures[@]}" >&2
  exit 1
fi
...
BASELINE_TB_DIR="$BASELINE_RUN/ibex_dv_out/build/tb"
if [[ ! -d "$BASELINE_TB_DIR" ]]; then
  echo "ERROR: baseline simulation did not produce a reusable compiled TB: $BASELINE_TB_DIR" >&2
  exit 1
fi
...
export BSDCOV_REUSE_TB_DIR="$BASELINE_TB_DIR"
"$SCRIPT_DIR/launch_sim.sh" \
...
baseline_tb_dir=$BASELINE_TB_DIR
project=$PROJ_DIR
fml_out=$FML_OUT
fsdb_root=$FSDB_ROOT
