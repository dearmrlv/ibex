...
def _source_setup_prefix(core_ibex: Path, cadence_xrun: str | None = None) -> str:
    parts = []
    setup = core_ibex / "setup_env.sh"
    if setup.exists():
        parts.append(f"source {shlex.quote(str(setup))}")
    if cadence_xrun:
        parts.append(f"export CADENCE_XRUN={shlex.quote(cadence_xrun)}")
...
def _compile_tb(*, repo_root: Path, run_dir: Path, bind_flists: list[Path], python: Path,
    ...
    command = _source_setup_prefix(core_ibex, env.get("CADENCE_XRUN")) + shlex.join(make_args)
...
def _run_rtl_chunk(*, repo_root: Path, tb_dir: Path, chunk: ChunkSpec, chunk_dir: Path,
                   ...
                   continue_on_assert: bool, covfile: Path | None, dry_run: bool) -> tuple[str, str]:
    if simulator != "xlm":
    ...
    cmd = [
        ...
        f"+signature_addr={os.environ.get('BSDCOV_SIGNATURE_ADDR', '8ffffffc')}",
        f"+test_timeout_s={os.environ.get('BSDCOV_RTL_TIMEOUT_S', '300')}",
    ]
    if os.environ.get("BSDCOV_DISABLE_COSIM", "1") != "0":
        cmd.append("+disable_cosim=1")
    if covfile is not None:
        cmd.extend(["-covfile", str(covfile)])
    if xrun_input:
...
def _run_one_chunk(repo_root: Path, python: Path, tb_dir: Path, chunk: ChunkSpec,
                   ...
                   continue_on_assert: bool, covfile: Path | None, dry_run: bool) -> ChunkResult:
...
def _find_canonical_io_sample_path(bsdcov_dir: Path, cone_name: str) -> Path | None:
    project_override = os.environ.get("BSDCOV_PROJECT_DIR", "").strip()
    if project_override:
        cones_root = Path(project_override).expanduser().resolve() / "cones"
    else:
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--covfile", type=Path, default=None, help="Optional Xcelium coverage configuration file passed to xrun -covfile.")
...
def main() -> int:
    ...
    covfile = args.covfile.expanduser().resolve() if args.covfile else None
    run_tag = args.run_tag or f"run.{_timestamp_tag()}.n{len(instr_seqs)}.step{args.cov_update}"
    sim_override = os.environ.get("BSDCOV_SIM_DIR", "").strip()
    sim_dir = Path(sim_override).expanduser().resolve() if sim_override else bsdcov_dir / "sim"
    ...
    if reuse_tb_dir:
        ...
        tb_dir = _compile_tb(repo_root=repo_root, run_dir=run_dir, bind_flists=bind_flists, python=python, simulator=args.simulator, iss=args.iss, seed=args.seed, force_compile=args.force_compile, fsdb=args.fsdb, covfile=covfile, dry_run=args.dry_run)
    results: list[ChunkResult] = []
    with ThreadPoolExecutor(max_workers=args.jobs) as ex:
        future_map = {ex.submit(_run_one_chunk, repo_root, python, tb_dir, chunk, run_dir / "chunks", args.simulator, args.rtl_test, args.isa, args.mabi, args.iss, fsdb_dir, args.continue_on_assert, covfile, args.dry_run): chunk for chunk in chunks}
