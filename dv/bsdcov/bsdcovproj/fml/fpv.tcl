...
analyze -sv12 \
  -f {/home/lvzhengyang/workspace/BSD-Cov/designs/ibex/dv/bsdcov/dut.f} \
  -f {/home/lvzhengyang/workspace/BSD-Cov/designs/ibex/dv/bsdcov/bsdcovproj/results/bsdcov.f}
elaborate -top {ibex_top} \
  ...
  -parameter BootAddr {32'h80000000}
clock clk_i
reset ~rst_ni
...
foreach p [get_property_list -include {type assert}] {
  if {[string match "*AST_BSDCOV_*" $p]} { lappend target_props $p }
}
if {[llength $target_props] == 0} { puts "ERROR: no generated BSD-Cov region assertions matched AST_BSDCOV_*"; exit 1 }
puts "INFO: BSD-Cov region assertion count = [llength $target_props]"
prove -property $target_props -asserts -force -per_property_time_limit $per_prop_time
report -property $target_props -results -detailed -file [file join $report_dir "fpv_report.$trace_name.txt"] -force
report -property $target_props -csv -include_type -file [file join $report_dir "fpv_report.$trace_name.csv"] -force
set cex_file [open [file join $report_dir "cex_properties.$trace_name.list"] w]
foreach p $target_props { set st [get_status $p]; if {$st eq "cex" || $st eq "ar_cex"} { puts $cex_file $p } }
close $cex_file
exit
