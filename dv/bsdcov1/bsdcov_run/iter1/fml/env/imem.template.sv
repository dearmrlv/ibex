...
module imem_if (
  ...
  always_comb begin
    if (rst_ni) begin
      ...
      // rdata/err are meaningful only when rvalid is high.
      if (instr_rvalid_i) begin
        assume(instr_rdata_i      == imem_rdata_o);
        assume(instr_err_i        == imem_err_o);
      end else begin
        assume(instr_err_i        == 1'b0);
      end

      // SecureIbex/MemECC: integrity bits must match the 32-bit instruction data.
      // Keep this combinational, because Ibex checks it combinationally.
